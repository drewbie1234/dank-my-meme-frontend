...
export const useWallet = () => {
    ...
    const fetchBalance = async (account) => {
        try {
            const response = await fetch(`https://194.124.43.95:443/api/getBalance?account=${account}`);
    ...
    const fetchEnsName = async (account) => {
        try {
            const response = await fetch(`https://194.124.43.95:443/api/getEns?account=${account}`);
            const data = await response.json();
            if (response.ok) {
                return data.ensName;
            } else {
